...
function blurHashToDataBuffer(hash: string, width: number, height: number, punch?: number): Buffer {
  const pixels = decode(hash, width, height, punch);
  const data = parsePixelsToBuffer(pixels, width, height);
  return data;
}

// thanks to https://github.com/wheany/js-png-encoder
function parsePixelsToBuffer(pixels: Uint8ClampedArray, width: number, height: number) {
  const pixelsString = Array.from(pixels)
    .map(byte => String.fromCharCode(byte))
    .join("");
  const pngString = generatePng(width, height, pixelsString);
  const buffer = Buffer.from(getPngArray(pngString));
  return buffer;
}
...
export default function getBlurData(blurHash: string, width = 160, height = 120, punch?: number) {
  if (blurHash == null || blurHash.length === 0) return "";
  return blurHashToDataBuffer(blurHash, width, height, punch);
}
